...
//                            LOGI("TapDetector: Tap detected");
    ...
    bool TapDetector::GetPointer(vecmath::Vec2 &v) {
        v.x = down_x_;
        v.y = down_y_;
        return true;
    }
...
//                        LOGI("DoubletapDetector: Doubletap detected");
    ...
    bool DoubletapDetector::GetPointer(vecmath::Vec2 &v) {
        v.x = last_tap_x_;
        v.y = last_tap_y_;
        return true;
    }
...
//--------------------------------------------------------------------------------
    ...
    bool PinchDetector::GetPointers(vecmath::Vec2 &v1, vecmath::Vec2 &v2) {
...
//                }
    ...
    bool DragDetector::GetPointer(vecmath::Vec2 &v) {
        v = vecmath::Vec2(last_x, last_y);
...
//--------------------------------------------------------------------------------
    ...
    GESTURE_STATE LongtapDetector::Detect(const AInputEvent *motion_event) {
        if (AMotionEvent_getPointerCount(motion_event) > 1) {
            // Only support single double tap
            return false;
        }
        ...
        switch (flags) {
            case AMOTION_EVENT_ACTION_DOWN: {
                ...
                if (eventTime - last_tap_time_ <= DOUBLE_TAP_TIMEOUT) {
                    ...
                    float y = AMotionEvent_getY(motion_event, 0) - last_tap_y_;
                    if (x * x + y * y < DOUBLE_TAP_SLOP * DOUBLE_TAP_SLOP * dp_factor_) {
//                        LOGI("LongtapDetector: Doubletap detected");
                        return GESTURE_STATE_ACTION;
                    }
                }
                break;
    ...
    void LongtapDetector::SetConfiguration(AConfiguration *config) {
        dp_factor_ = 160.f / AConfiguration_getDensity(config);
        tap_detector_.SetConfiguration(config);
    }

    bool LongtapDetector::GetPointer(vecmath::Vec2 &v) {
        v.x = last_tap_x_;
        v.y = last_tap_y_;
        return true;
    }
