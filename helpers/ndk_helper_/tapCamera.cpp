...
TapCamera::TapCamera()
  ...
  vec_flip_ = vecmath::Vec2(1.f, -1.f);
  flip_z_ = -1.f;
  vec_pinch_transform_factor_ = Vec3(1.f, 1.f, 1.f);

  vec_ball_center_ = vecmath::Vec2(0, 0);
  vec_ball_now_ = vecmath::Vec2(0, 0);
  vec_ball_down_ = vecmath::Vec2(0, 0);

  vec_pinch_start_ = vecmath::Vec2(0, 0);
  vec_pinch_start_center_ = vecmath::Vec2(0, 0);

  vec_flip_ = vecmath::Vec2(0, 0);
}
...
void TapCamera::Update() {
  if (momentum_) {
    ...
    // Momentum rotation
    vecmath::Vec2 v = vec_drag_delta_;
    BeginDrag(vecmath::Vec2());  // NOTE:This call reset _VDragDelta
    Drag(v * vec_flip_);
...
// Drag control
//----------------------------------------------------------
void TapCamera::BeginDrag(const vecmath::Vec2& v) {
  if (dragging_) EndDrag();

  if (pinching_) EndPinch();

  vecmath::Vec2 vec = v * vec_flip_;
  vec_ball_now_ = vec;
...
void TapCamera::Drag(const vecmath::Vec2& v) {
  if (!dragging_) return;
...
// Pinch controll
//----------------------------------------------------------
void TapCamera::BeginPinch(const vecmath::Vec2& v1, const vecmath::Vec2& v2) {
  if (dragging_) EndDrag();

  if (pinching_) EndPinch();

  BeginDrag(vecmath::Vec2());

  vec_pinch_start_center_ = (v1 + v2) / 2.f;

  vecmath::Vec2 vec = v1 - v2;
  float x_diff;
  float y_diff;
...
void TapCamera::Pinch(const vecmath::Vec2& v1, const vecmath::Vec2& v2) {
  if (!pinching_) return;
...
Vec3 TapCamera::PointOnSphere(vecmath::Vec2& point) {
  Vec3 ball_mouse;
  float mag;
  vecmath::Vec2 vec = (point - vec_ball_center_) / ball_radius_;
  mag = vec.Dot(vec);
  if (mag > 1.f) {
