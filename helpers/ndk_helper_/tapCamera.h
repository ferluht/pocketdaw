...
class TapCamera {
 private:
  // Trackball
  vecmath::Vec2 vec_ball_center_;
  float ball_radius_;
  Quaternion quat_ball_now_;
  Quaternion quat_ball_down_;
  vecmath::Vec2 vec_ball_now_;
  vecmath::Vec2 vec_ball_down_;
  ...
  // Pinch related info
  vecmath::Vec2 vec_pinch_start_;
  vecmath::Vec2 vec_pinch_start_center_;
  ...
  bool momentum_;
  double time_stamp_;
  vecmath::Vec2 vec_drag_delta_;
  vecmath::Vec2 vec_last_input_;
  Vec3 vec_offset_last_;
  Vec3 vec_offset_delta_;
  float momemtum_steps_;

  vecmath::Vec2 vec_flip_;
  float flip_z_;
  ...
  Vec3 PointOnSphere(vecmath::Vec2& point);
  void BallUpdate();
  void InitParameters();
 ...
 public:
  TapCamera();
  virtual ~TapCamera();
  void BeginDrag(const vecmath::Vec2& vec);
  void EndDrag();
  void Drag(const vecmath::Vec2& vec);
  void Update();
  ...
  void BeginPinch(const vecmath::Vec2& v1, const vecmath::Vec2& v2);
  void EndPinch();
  void Pinch(const vecmath::Vec2& v1, const vecmath::Vec2& v2);

  void SetFlip(const float x, const float y, const float z) {
    vec_flip_ = vecmath::Vec2(x, y);
    flip_z_ = z;
  }
