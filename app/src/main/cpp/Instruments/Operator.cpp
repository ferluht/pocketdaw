...
Operator::Operator(){
    ...
    for (int i = 0; i < 4; i++) {
        enc_ratios[i] = new Encoder(L"level 1", 0, [this, i](float value) {
...
{
    float sample = (sin(state->phases[0]) * sin(state->phases[1]) + sin(state->phases[2]) * sin(state->phases[3]))/2; //(sin(fmod(beat, M_PI)) + 1)/4; //* sin(state->phase2); //(sin(fmod(beat, M_PI)) + 1)/4;// * sin(state->phase2) + cnt;
